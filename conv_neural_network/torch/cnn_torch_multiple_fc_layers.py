...
# Import keras for Conv2DTranspose
...
class Convolutional_Neural_Network_Multiple_FC_Layers():
    ...
    def max_pooling_batch(self, stride, activation_images, max_pool_images):
        ...
        pooled_output, indicies = F.max_pool2d(activation_images, pool_size, stride=stride, return_indices=True)
        # print("Pooled Output Shape: ",pooled_output.shape)
        # print("Indicies Shape: ",indicies.shape)
        max_pool_images[:] = pooled_output.permute(0, 2, 3, 1)
        self.max_pool_indices.append(indicies.permute(0, 2, 3, 1))
    ...
    def backpropagation_batch(self, X, y, learning_rate):
        ...
        # print("Output Delta: ",d_output_delta.shape)
        ...
        for i in range(self.conv_layers.size() - 1, -1, -1):
            ...
            max_pool_indices = self.max_pool_indices[i]
            # Get shapes
            batch_size, curr_h, curr_w, channels = conv.activation_images.shape
            
            # Convert to NCHW format and unpool
            backprop_derivative_images = F.max_unpool2d(
                delta_loss_over_delta_max_pool_images.permute(0, 3, 1, 2),
                max_pool_indices.permute(0, 3, 1, 2),
                kernel_size=2,
                stride=2,
                output_size=(batch_size, channels, curr_h, curr_w)
            ).permute(0, 2, 3, 1)
            ...
            # print("\nD Kernels Shape: ",d_kernels.shape)
            # print("D Bias Shape: ",d_bias.shape)
            ...
            # d_kernels = F.conv2d(input_layer.permute(0, 3, 1, 2), backprop_activation_images.permute(0, 3, 1, 2), stride=conv.stride)
            # print("D Kernels Shape: ",d_kernels.shape)
            # kk
    ...
    def train(self, X, y, epochs, learning_rate, batch_size):
        ...
        for epoch in range(epochs):
            ...
            if validation_accuracy_cats == 100.0 and validation_accuracy_dogs == 100.0:
                ...
                break
            elif validation_accuracy_cats > 90.0 and validation_accuracy_dogs > 90.0:
